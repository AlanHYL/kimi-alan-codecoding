...
项目创建完成后，按 AI 给的提示操作即可。根据技术栈不同，运行方式也不同：
...
**一键启动（如果生成了）:**
双击 `start.bat`（Windows）或运行 `sh start.sh`（Mac/Linux）即可。
...
- 使用项目模板，自动匹配最合适的方案
- 展示模板功能 + AI 的技术假设，**一次确认即可**
- 跳过架构设计环节，直接用模板内置方案
- 如果需求没有匹配的模板，**自动降级**为标准流程（不会卡住）
...
AlanCodecoding 交付的代码经过严格质量控制，不是"能跑就行"的代码：
...
| 类别 | 强制要求 |
|---|---|
| 🔒 **零硬编码** | 端口、数据库连接、API密钥等全部走环境变量，不能在代码中写死 |
| 🔒 **认证安全** | 非公开 API 必须有认证校验，CORS 配白名单，请求限流 |
| 🔒 **输入校验** | 所有用户输入必须用 Joi/Zod 校验，禁止手动 if 拼接 |
| 📊 **日志** | 使用 winston/pino 结构化日志，禁止 console.log |
| 📊 **错误处理** | 统一错误中间件，禁止向客户端返回堆栈信息 |
| ⚡ **优雅关闭** | SIGTERM/SIGINT 处理，确保不丢数据 |
