...
以下要求不是"建议"，是**必须执行**。不满足这些条件的代码不允许提交：

**1. 零硬编码**
- 所有配置值必须从 `process.env` 读取（数据库连接、API密钥、URL、端口等）
- 所有业务常量必须在文件顶部定义，不能散落在代码中
- 示例：`const PORT = process.env.PORT || 3000;` ✅  /  `app.listen(3000);` ❌

**2. 安全**
- 用户认证：任何非公开端点必须有认证校验
- 输入校验：所有用户输入必须校验（Joi/Zod/express-validator），不能只靠前端
- SQL 注入防护：必须使用参数化查询，禁止拼接 SQL
- CORS：必须配置白名单，不能 `app.use(cors())` 放通所有来源
- 请求限流：公开 API 必须配置 rate-limit
- 错误信息：禁止向客户端返回堆栈信息（`res.json({ error: err.message })` ✅ / `res.send(err.stack)` ❌）

**3. 可观测性**
- 必须使用结构化日志库（winston/pino），禁止 `console.log`
- 必须使用统一错误处理中间件（`app.use((err, req, res, next) => {...})`）
- 必须提供 `/health` 端点

**4. 可靠性**
- 必须处理优雅关闭（`process.on('SIGTERM', shutdown)` 关闭数据库连接）
- API 端点必须设置超时
- 数据库查询必须使用连接池或重试机制

**5. 配置管理**
- 必须提供 `.env.example` 说明所有环境变量
- 密钥/密码必须走环境变量，不能写在代码或配置文件中
